...
class DisjointKeys(DataSet):
    ...
    @property
    def sources(self) -> list[ISource]:
        return [Source.json_data(f'/year=2022/month={m:02}/{d:02}.json', { f"key{d:02}": d })
                for d in range(1, 3) for m in range(1, 13)]
...
class NonHiveDisjointKeys(DataSet):
    ...
    @property
    def sources(self) -> list[ISource]:
        return [Source.json_data(f'/2022/{m:02}/{d:02}.json', { f"key{d:02}": d })
...
class OverlappingKeys(DataSet):
    ...
    @property
    def sources(self) -> list[ISource]:
        return [Source.json_data(f'/year=2022/month={m:02}/{d:02}.json',
                   {
                       "id": 10*m + d,
